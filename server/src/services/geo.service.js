...
const findNearbyResources = async (lat, lng, radiusKm, category) => {
  const categoryFilter = category
    ? Prisma.sql`AND category = ${category}::"Category"`
    : Prisma.empty;
  ...
  return prisma.$queryRaw`
    SELECT 
      id, "offeredById", "category", "description", "quantityAvailable", "status",
      ST_Y(location::geometry) as lat, 
      ST_X(location::geometry) as lng,
      ST_Distance(location, ST_MakePoint(${lng}, ${lat})::geography) / 1000 AS "distanceKm"
    FROM "resources"
...
module.exports = { findNearbyRequests, findNearbyResources };
